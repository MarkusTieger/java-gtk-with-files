...
class JnaAttrFontFeatures {

    private static long _size = -1;
    public static long allocateStructure() {
        if (_size < 0) {
            _size = new Fields().size();
            System.out.println("AttrFontFeatures size: " + _size + " bytes");
        }
        return ch.bailu.gtk.lib.jna.CLib.allocate(_size);
    }
    
    @com.sun.jna.Structure.FieldOrder({AttrFontFeatures.ATTR, AttrFontFeatures.FEATURES})
